...
import { BadgeCheck, PawPrint, Building2, Heart, Home, ShieldHalf, ShieldAlert, Search, Clock, Briefcase } from "lucide-react";
...
const META: Record<AccountType, { label: string; tone: string; Icon: any }> = {
  ...
  provider: { label: "Service provider", tone: "bg-primary/10 text-primary border-primary/30", Icon: Briefcase },
};

const NON_VERIFIABLE: AccountType[] = ["pet_parent", "buyer", "provider"];
...
export const SellerBadge = ({
  ...
  const m = META[t] ?? META.pet_parent;
  const Icon = m.Icon;
  const showVerification = !NON_VERIFIABLE.includes(t);
  return (
    <Badge className={`border ${m.tone} gap-1 font-normal ${className ?? ""}`}>
      <Icon className="h-3 w-3" />
      {m.label}
      {verified && showVerification && <BadgeCheck className="h-3 w-3 text-leaf" />}
      {!verified && pending && showVerification && (

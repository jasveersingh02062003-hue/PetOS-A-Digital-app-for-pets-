...
const MedDialog = ({ open, onOpenChange, petId }: { open: boolean; onOpenChange: (b: boolean) => void; petId: string }) => {
  ...
  const [form, setForm] = useState({ name: "", dose: "", route: "oral", frequency: "", reason: "", prescribing_vet: "", start_on: today, end_on: "" });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    qc.invalidateQueries({ queryKey: ["meds", petId] });
    onOpenChange(false);
    setForm({ name: "", dose: "", route: "oral", frequency: "", reason: "", prescribing_vet: "", start_on: today, end_on: "" });
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-3">
          <Field label="Medication name" value={form.name} onChange={(v) => setForm({ ...form, name: v })} placeholder="Apoquel, Prednisone…" />
          <div className="grid grid-cols-2 gap-3">
            <Field label="Dose" value={form.dose} onChange={(v) => setForm({ ...form, dose: v })} placeholder="5mg" />
            <Field label="Route" value={form.route} onChange={(v) => setForm({ ...form, route: v })} placeholder="oral / SC / IM" />
          </div>
          <Field label="Frequency" value={form.frequency} onChange={(v) => setForm({ ...form, frequency: v })} placeholder="Twice daily" />
          <Field label="Reason / condition" value={form.reason} onChange={(v) => setForm({ ...form, reason: v })} placeholder="Allergy, infection…" />
          <Field label="Prescribing vet" value={form.prescribing_vet} onChange={(v) => setForm({ ...form, prescribing_vet: v })} />
          <div className="grid grid-cols-2 gap-3">
            <Field label="Start date" type="date" value={form.start_on} onChange={(v) => setForm({ ...form, start_on: v })} />
            <Field label="End date" type="date" value={form.end_on} onChange={(v) => setForm({ ...form, end_on: v })} />
          </div>
